...
bool tarFile(int file_count, std::string file_names[], std::string file_frontname, const char * output_path)
{
    fprintf(stderr, "file_count=%i,output_path=%s", file_count, output_path);
    Tar tar(output_path);
    
    /* Add existing files to tar archive*/
    for (int i = 0; i < file_count; i++) {
        std::string str = file_names[i];
        __SIZE_TYPE__ pos = str.find_last_of("/");
        std::string filename(str.substr(pos + 1));

        std::string pathNoFileName(str.substr(0, pos));
        pos = pathNoFileName.find_last_of("/");
        std::string frontname1(pathNoFileName.substr(pos + 1));

        std::string nameInArchive = file_frontname.c_str();
        nameInArchive += "/";
        nameInArchive += frontname1;
        nameInArchive += "/";
        nameInArchive += filename;
        
        tar.putFile(file_names[i].c_str(), nameInArchive.c_str());
    }
